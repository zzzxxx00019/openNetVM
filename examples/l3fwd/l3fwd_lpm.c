...
#define IPV4_L3FWD_LPM_NUM_ROUTES (sizeof(ipv4_l3fwd_lpm_route_array) / sizeof(ipv4_l3fwd_lpm_route_array[0]))
...
setup_lpm_request(void) {
        ...
        struct lpm_request *l3switch_req = (struct lpm_request *)rte_malloc(NULL, sizeof(struct lpm_request), 0);
        if (!l3switch_req)
                return NULL;
        ...
        for (i = 0; i < IPV4_L3FWD_LPM_NUM_ROUTES; i++) {
                ...
                if (get_initialized_ports(ipv4_l3fwd_lpm_route_array[i].if_out) == 0)
                        continue;
                ret = rte_lpm_add(lpm_tbl, ipv4_l3fwd_lpm_route_array[i].ip, ipv4_l3fwd_lpm_route_array[i].depth,
                ...
                printf("\nLPM: Adding route 0x%08x / %d (%d)\n", (unsigned)ipv4_l3fwd_lpm_route_array[i].ip,
                       ipv4_l3fwd_lpm_route_array[i].depth, ipv4_l3fwd_lpm_route_array[i].if_out);
                printf("IP: %" PRIu8 ".%" PRIu8 ".%" PRIu8 ".%" PRIu8, ipv4_l3fwd_lpm_route_array[i].ip >> 24 & 0xFF,
                       (ipv4_l3fwd_lpm_route_array[i].ip >> 16) & 0xFF, (ipv4_l3fwd_lpm_route_array[i].ip >> 8) & 0xFF,
                       (ipv4_l3fwd_lpm_route_array[i].ip) & 0xFF);
        }
        return lpm_tbl;
}
...
setup_lpm(struct state_info *stats) {
        ...
        for (i = 0; i < IPV4_L3FWD_LPM_NUM_ROUTES; i++) {
                ...
                ret = rte_lpm_add(stats->lpm_tbl, ipv4_l3fwd_lpm_route_array[i].ip, ipv4_l3fwd_lpm_route_array[i].depth,
                                  ipv4_l3fwd_lpm_route_array[i].if_out);
...
lpm_get_ipv4_dst_port(void *ipv4_hdr, uint16_t portid, struct state_info *stats) {
        uint32_t next_hop;
        return (uint16_t)((rte_lpm_lookup(stats->lpm_tbl, rte_be_to_cpu_32(((struct rte_ipv4_hdr *)ipv4_hdr)->dst_addr),
                                          &next_hop) == 0)
                              ? next_hop
                              : portid);
}
